...
신규 세션의 모든 ace report frontmatter에 다음 필드 의무 기록:
...
### 본 역할 지표 (5건)
- `rfrm_trg` (Y/N) — **core** — 리프레임 트리거: Master 가정 흔들거나 결정축 재구성 성공
- `ctx_car` (ratio 0~1) — **core** — 컨텍스트 승계: 선행 발언 누락 없이 합성한 비율
- `orc_hit` (ratio 0~1) — 오케스트레이션 적중: 역할 호출 타이밍·순서 판정 적중률
- `mst_fr` (ratio 0~1) — Master 재수정 회피율: 첫 권고가 수정 없이 수용된 비율
- `ang_nov` (0-5) — 각도 신규성: 직전 세션 대비 신규 프레이밍 축 수
