...
- **발동 조건**:
  - (a) `/jobs-framing` skill 명시 호출 (Master 또는 Nexus)
  - (b) Master 명시 호출 (`jobs`, `잡스` 등)
  - (c) Nexus orchestration 판단에 의한 호출
- **callerScope: `master | nexus`** — 다른 역할이 직접 호출 불가. Nexus 경유 필수 (D-130 single-orchestration 정합).
- **자동 트리거 없음** — 명시 호출만 (D-130).
...
**본질에 도달하기까지 99%의 옵션을 잘라낸다. 사용자가 frame을 보는 순간 "이게 답이다"라고 느끼게 만든다.**
