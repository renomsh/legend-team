...
> - 역할 정책: `memory/roles/policies/role-sage.md` (P3 작성 예정)
...
본 시점 Sage same-session 격리는 `pre-tool-use-task-sage-gate.js` hook + `## ROLE: sage` 마커로 강제되나, 마커 위조 시 우회 가능성 잔존. 후속 토픽에서 `subagent_type === 'role-sage'` AND marker 일치 이중 검증 + PostToolUse 재검증 hook으로 처리 예정 (D-126 본문 박제, PD-052 동형 사례).
