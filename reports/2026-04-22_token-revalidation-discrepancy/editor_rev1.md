...
## 대시보드 표시안 (확정 — v2, Master 재조정 반영)

**핵심 재조정**: Master 실결제는 Max 정액제($100 + $120 버퍼 추가) → 거의 상수. 관심은 "결제액 트래킹"이 아닌 **"사용량 + 효율"**.

### KPI 헤더 (Primary = 사용량 + 효율)
```
I/O 토큰(누적): 7.09M    캐시 효율: 93.7%    세션: 71    세션당 평균 I/O: 99.8K
```

### 보조 스트립 (Secondary = 실결제 I/O 기준)
```
실결제 $104.28 │ 세션당 평균 $1.47 │ 정액($100+버퍼$120)
```
