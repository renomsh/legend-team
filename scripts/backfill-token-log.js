    ...
    for await (const line of rl) {
        ...
        try {
            ...
            if (obj.type === 'user') {
                ...
                const isMasterTurn = typeof c === 'string' ||
...
async function findFirstOpenTs(transcriptPath) {
    const rl = readline.createInterface({
        input: fs.createReadStream(transcriptPath, { encoding: 'utf-8' }),
        crlfDelay: Infinity,
    });
...
async function main() {
    ...
    for (const d of dirs) {
        ...
        for (const f of fs.readdirSync(dirPath)) {
            ...
            if (stat.size < 1000)
                continue;
            if (stat.mtime < new Date('2026-04-01'))
                continue;
            ...
            for (const s of sessions) {
                ...
                if (!best || diff < best.diff)
                    best = { sessionId: s.sessionId, diff };
            }
            if (!best)
                continue;
...
//# sourceMappingURL=backfill-token-log.js.map