...
exports.loadCurrentSession = loadCurrentSession;
exports.filterRoleMetrics = filterRoleMetrics;
...
function filterRoleMetrics(role) {
    return (0, self_scores_writer_1.loadRegistry)().metrics.filter(m => m.role === role && m.lifecycleState === "active");
}
function ask(rl, q) {
    return new Promise(resolve => rl.question(q, ans => resolve(ans.trim())));
}
async function pickRole(rl) {
    while (true) {
        const ans = await ask(rl, `역할 선택 [${VALID_ROLES.join("/")}]: `);
        if (VALID_ROLES.includes(ans))
            return ans;
        console.log(`  유효하지 않습니다. 다음 중 선택: ${VALID_ROLES.join(", ")}`);
    }
}
async function collectScore(rl, m) {
    const ans = await ask(rl, `  [${m.id}] (${m.scale}, ${m.construct.slice(0, 40)}) 점수 입력 (Enter=skip): `);
    if (!ans)
        return null;
    return ans;
}
async function runInteractive(opts) {
    ...
    const topicType = cs.topicType || "implementation";
    ...
    try {
        ...
        for (const m of metrics) {
            ...
            if (opts.dryRun) {
                ...
                try {
                    const rec = (0, self_scores_writer_1.appendScore)(input);
...
if (require.main === module) {
    ...
    const opts = { sessionId: args.sessionId };
    if (args.dryRun)
        opts.dryRun = true;
    runInteractive(opts).then(n => process.exit(n >= 0 ? 0 : 1), err => {
        console.error(err);
        process.exit(1);
    });
}
//# sourceMappingURL=batch-score-helper.js.map