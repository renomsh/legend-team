...
function validateMetric(m, source) {
...
function fillStub(m, source, lifecycle = "active") {
    const filled = { ...m };
    if (!filled.role && filled.id)
        filled.role = String(filled.id).split(".")[0];
    if (!filled.scope)
        filled.scope = "role";
    if (!filled.axis)
        filled.axis = "quality";
    ...
    if (!filled.scale || !validScales.includes(filled.scale))
        filled.scale = "0-5";
    if (!filled.polarity)
        filled.polarity = "higher-better";
    if (!filled.construct)
        filled.construct = `(stub) ${filled.id} — definition pending; preserved from ${source}`;
    ...
    if (!filled.validityCheck)
        filled.validityCheck = "monthly";
    ...
    if (!filled.rater || typeof filled.rater !== "object")
        filled.rater = { type: "self", by: filled.role };
    if (!filled.raterWeights)
        filled.raterWeights = { [filled.role]: 1 };
    if (!filled.timing)
        filled.timing = "immediate";
    if (!filled.aggregation)
        filled.aggregation = "all-sessions";
    if (filled.baselineSessions === undefined)
        filled.baselineSessions = 10;
    if (!filled.lifecycleState)
        filled.lifecycleState = lifecycle;
    if (!filled.inputPriority)
        filled.inputPriority = "extended";
    if (!filled.defaultStrategy)
        filled.defaultStrategy = "previous-session-value";
    if (!filled.missingPenalty)
        filled.missingPenalty = "silent";
...
function collectAggregateIds() {
    ...
    if (fs.existsSync(AGGREGATE_PATH)) {
        try {
            ...
            console.warn(`[compile-metrics-registry] WARN: aggregate read failed: ${e.message}`);
        }
    }
...
for (const m of metrics) {
    if (m.axis in axisDistribution)
        axisDistribution[m.axis]++;
}
...
(0, write_atomic_1.writeAtomic)(REGISTRY_PATH, JSON.stringify(compiled, null, 2) + "\n");
(0, write_atomic_1.writeAtomic)(HISTORY_PATH, JSON.stringify(compiled, null, 2) + "\n");
...
//# sourceMappingURL=compile-metrics-registry.js.map