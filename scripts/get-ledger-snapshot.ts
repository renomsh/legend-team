...
function getSnapshot(topicId: string, limit: number): LedgerSnapshot {
  ...
  const ledger: { decisions: Decision[] } = JSON.parse(raw);
  const all = ledger.decisions || [];

  // 1. 현 topicId 관련 결정 전체
  const topicRelated = all.filter(
    (d) =>
      d.topic === topicId ||
  ...
  return {
    ...
    limit,
    escapeHatch:
      '충돌 결정 의심 시 decision_ledger.json 전문 조회 가능 (escape hatch). ' +
...
if (args.length === 0 || args[0] === '--help') {
  console.error(
    'Usage: npx ts-node scripts/get-ledger-snapshot.ts <topicId> [--limit=30]\n' +
