...
exports.isLifecycleAware = isLifecycleAware;
exports.canAutoClose = canAutoClose;
exports.validateLifecycleSchema = validateLifecycleSchema;
exports.matchesResolveCondition = matchesResolveCondition;
exports.scanGitLog = scanGitLog;
/** topicType 존재 여부 기반 판정 */
function isLifecycleAware(topic) {
    return topic.topicType !== undefined;
}
/** framing 토픽 자동 종결 가능 여부: 모든 children completed */
function canAutoClose(topic, childStatuses) {
    ...
    const openChildren = children.filter((cid) => childStatuses[cid] !== 'completed' && childStatuses[cid] !== 'closed');
...
function validateLifecycleSchema(topics) {
    const issues = [];
    const byId = new Map(topics.map((t) => [t.id, t]));
    for (const t of topics) {
        if (t.topicType === undefined)
            continue;
...
function matchesResolveCondition(condition, signal) {
 ...
 */
function scanGitLog(root) {
    const result = new Map();
    try {
        const { execSync } = require('child_process');
        const output = execSync('git log --oneline --all --since="6 months ago"', {
            cwd: root,
            encoding: 'utf-8',
            timeout: 10000,
        ...
        for (const line of output.split('\n')) {
            ...
            for (const raw of pdMatches) {
                ...
                if (!result.has(pdId))
                    result.set(pdId, []);
                const commitType = /^session[_ ]?end:/i.test(message) ? 'session-end' : 'implementation';
...
//# sourceMappingURL=topic-lifecycle.js.map