...
exports.lintAccentOnly = lintAccentOnly;
/**
...
function walk(dir, root, hits) {
    for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (ent.isDirectory()) {
            if (EXCLUDE_DIRS.includes(ent.name))
                continue;
            walk(p, root, hits);
            continue;
        }
        if (!SCAN_EXTENSIONS.includes(path.extname(ent.name).toLowerCase()))
            continue;
...
function lintAccentOnly(scanDir) {
    const hits = [];
...
//# sourceMappingURL=lint-accent-only.js.map