...
function buildHookInput(role, agentId, selfScoresYaml) {
    return {
        tool_name: 'Task',
        tool_input: {
...
function spawnHook(hookInput, pendingTurnsPath) {
    return new Promise((resolve) => {
        ...
        const child = (0, child_process_1.spawn)('node', [HOOK_PATH], { env });
        let stderr = '';
...
async function runOptionA(sessionId) {
    ...
    const agentIds = roles.map((_, i) => `agent_${sessionId}_${Date.now()}_${i}`);
...
function runOptionB() {
    ...
    const trials = testCases.map((tc, i) => {
...
async function main() {
    ...
    // GATE α
    const gate = gateAlpha({ matchRate: optAResult.matchRate, corruptCount: optAResult.corruptCount }, { truncationRate: optBResult.truncationRate });
    console.log('\n=== GATE α ===');
    ...
    // cleanup tmp
    try {
        fs.rmSync(SPIKE_TMP, { recursive: true, force: true });
    }
    catch { }
...
//# sourceMappingURL=spike-p1-option-ab-compare.js.map