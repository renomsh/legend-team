...
interface HookInput {
  ...
  tool_response: { content: { type: string; text: string }[] };
  cwd: string;
  session_id: string;
}

function buildHookInput(role: string, marker: string): HookInput {
  return {
    tool_name: 'Task',
    tool_input: {
...
function spawnHook(input: HookInput, env: NodeJS.ProcessEnv): Promise<SpawnResult> {
  return new Promise((resolve) => {
    const startMs = Date.now();
    const child: ChildProcess = spawn('node', [HOOK_PATH], {
      cwd: CWD,
      env: { ...process.env, ...env },
...
interface ScenarioResult {
  ...
  description: string;
  concurrent: number;
  expectedDelta: number; // 예상 turns 추가 수
  observedDelta: number; // 실제 turns 추가 수
  duplicateTurnIdx: number[]; // 중복된 turnIdx 값들
  lostWrites: number; // 예상 - 실제 (>0이면 lost)
  raceDetected: boolean;
  raceReason: string[];
  durationMs: number;
  timingOverlaps: number; // hook간 실행 시간 겹침 카운트
  spawnResults: SpawnResult[];
}

function turnsBefore(): number {
  const sess = readJson(SESSION_PATH);
  return Array.isArray(sess.turns) ? sess.turns.length : 0;
}

function detectDuplicates(turns: any[]): number[] {
...
function analyzeTimingOverlaps(timingLog: string): number {
  if (!fs.existsSync(timingLog)) return 0;
  const lines = fs
  ...
  const intervals: { pid: number; readNs: bigint; writeNs: bigint }[] = [];
  const byPid = new Map<number, any>();
  for (const e of lines) {
    if (!byPid.has(e.pid)) byPid.set(e.pid, {});
    const slot = byPid.get(e.pid);
    if (e.phase === 'turns-read-before') slot.read = BigInt(e.ts);
    if (e.phase === 'turns-write-after') slot.write = BigInt(e.ts);
  }
...
async function runScenario(
  ...
  description: string,
  concurrent: number,
  options: { externalWriteDuringRun?: boolean } = {}
): Promise<ScenarioResult> {
  // 시나리오 시작 시 timing log truncate (시나리오별 격리)
  if (fs.existsSync(TIMING_LOG)) fs.unlinkSync(TIMING_LOG);

  const before = turnsBefore();
  const startMs = Date.now();

  const inputs = Array.from({ length: concurrent }, (_, i) =>
    buildHookInput('dev', `${id}_${i}`)
  );

  // Promise.all로 동시 spawn — 같은 tick에서 child_process spawn 호출
  const promiseAll = Promise.all(
    inputs.map((inp) => spawnHook(inp, { SPIKE_R6_LOG: TIMING_LOG }))
  );

  // S5: 외부 프로세스가 동시에 session 파일에 write 시도 (외부 충돌)
  let externalWritePromise: Promise<void> | null = null;
  ...
  if (externalWritePromise) await externalWritePromise;
  ...
  const raceReason: string[] = [];
  if (lostWrites > 0) raceReason.push(`lost_writes=${lostWrites}`);
...
async function main() {
  ...
  try {
    ...
    results.push(
      await runScenario('S5', 'hook 5개 + 외부 write 병렬', 5, {
  ...
  for (const r of results) {
    console.log(
      `  ${r.id}: race=${r.raceDetected} | expected=${r.expectedDelta} observed=${r.observedDelta} dup=${r.duplicateTurnIdx.length} overlaps=${r.timingOverlaps}${r.raceReason.length ? ' | ' + r.raceReason.join(',') : ''}`
    );
  }
}
...
async function runScenarioStaggered(
  ...
  for (let i = 0; i < concurrent; i++) {
    ...
    if (i < concurrent - 1) await new Promise((r) => setTimeout(r, intervalMs));
  }
...
// callable export (dev_verify_and_callable)
