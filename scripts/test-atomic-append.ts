...
function targetByteSize(size: LineSize): number {
  if (size === "small") return 100;
  if (size === "medium") return 4 * 1024;
  return 16 * 1024;
}

function makePadding(byteTarget: number, headerLen: number): string {
...
if (process.argv.includes(WORKER_FLAG)) {
  ...
  const size = args.size as LineSize;
  ...
  // Notify parent
  if (process.send) process.send({ workerDone: id });
  process.exit(0);
}
...
export async function runTest(opts: TestOptions): Promise<TestResult> {
  ...
  const file = path.join(
    outDir,
    `append-${opts.size}-w${opts.workers}-l${opts.lines}${trialTag}-${Date.now()}.jsonl`,
  );
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    for (let i = 0; i < opts.workers; i++) {
      ...
      child.on("message", (msg: any) => {
        if (msg && msg.workerDone) doneIds.push(String(msg.workerDone));
      });
      child.on("exit", (code) => {
        exitedCount++;
        if (code !== 0 && !errored) {
        ...
        if (exitedCount === opts.workers && !errored) resolve();
      });
  ...
  const seenPerWorker: Record<string, Set<number>> = {};
  const lastSeqPerWorker: Record<string, number> = {};
  const outOfOrderPerWorker: Record<string, number> = {};
  let duplicates = 0;
  const order: string[] = []; // worker id sequence as parsed
  const notes: string[] = [];
  ...
  for (let i = 1; i < order.length; i++) {
    if (order[i] !== order[i - 1]) interleavePoints++;
  }

  // Missing per worker
  const missingPerWorker: Record<string, number> = {};
  ...
  const verdict: "OK" | "FAIL" =
    lineCountOk && noParseFail && noMissing && noDup ? "OK" : "FAIL";

  if (!lineCountOk) notes.push(`line count mismatch: expected ${expectedLines}, got ${actualLines}`);
  if (!noParseFail) notes.push(`${jsonParseFails} lines failed JSON.parse (likely line splits/interleave)`);
  if (corruptOrSplit > 0) notes.push(`${corruptOrSplit} lines show inter-worker byte interleave inside a single line`);
  if (!noMissing) notes.push(`missing seqs: ${JSON.stringify(missingPerWorker)}`);
  if (!noDup) notes.push(`${duplicates} duplicate (worker,seq) tuples observed`);
...
async function mainCli() {
  ...
  if (all) {
    const sizes: LineSize[] = ["small", "medium", "large"];
    ...
    for (const r of results) {
      ...
      console.log(
        `${r.size}\t${"?"}\t${r.expectedLines}\t${r.actualLines}\t${r.jsonParseFails}\t${r.corruptOrSplit}\t${missTotal}\t${r.duplicates}\t${r.interleavePoints}\t${r.verdict}`,
      );
    }
    ...
    const size = (args.size as LineSize) ?? "small";
    const workers = Number(args.workers ?? 4);
    const lines = Number(args.lines ?? 1000);
