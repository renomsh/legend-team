...
function buildHookInput(role, agentId, selfScoresYaml, cwdOverride) {
    return {
        tool_name: 'Task',
        tool_input: {
...
function makeSession(turnPushMode, existingTurns = []) {
    return {
...
function setupFixture(sessionObj) {
...
async function t6() {
    ...
    const inputs = roles.map((role, i) => buildHookInput(role, agentIds[i], `metric: ${i}`, FIXTURE_DIR));
...
//# sourceMappingURL=test-p3-turn-push-mode.js.map