...
function writePending(dir, sessionId, entries) {
...
function spawnFinalize(dir) {
    return new Promise((resolve) => {
        const child = (0, child_process_1.spawn)('node', [FINALIZE_PATH], {
            env: {
                ...process.env,
                FINALIZE_CWD: dir,
...
async function c1() {
    ...
    const noOrphanJoin = !result.gaps?.some((g) => g.type === 'nexus-crash-recovery');
    assert('C1-no-crash-recovery-gap', noOrphanJoin, `gaps=${JSON.stringify(result.gaps?.map((g) => g.type))}`);
...
async function c2() {
    ...
    const { stderr } = await spawnFinalize(dir);
    const result = JSON.parse(fs.readFileSync(path.join(dir, 'memory', 'sessions', 'current_session.json'), 'utf8'));
    assert('C2-no-crash-recovery-gap', !result.gaps?.some((g) => g.type === 'nexus-crash-recovery'), `gaps=${JSON.stringify(result.gaps?.map((g) => g.type))}`);
...
async function c3() {
    ...
    const crashTurns = (result.turns || []).filter((t) => t._crashRecovery);
    ...
    assert('C3-gap-boxed', result.gaps?.some((g) => g.type === 'nexus-crash-recovery'), `gaps=${JSON.stringify(result.gaps?.map((g) => g.type))}`);
    ...
    assert('C3-archive-exists', archiveFiles.some(f => f.startsWith('pending_turns_test_session')), `archive files=${archiveFiles.join(',')}`);
}
...
async function c4() {
    ...
    assert('C4-invalid-not-joined', !crashTurns.some((t) => t.role === 'evil'), 'evil role must not appear');
    assert('C4-invalid-origin-gap', result.gaps?.some((g) => g.type === 'nexus-crash-recovery-invalid-origin'), `gaps=${JSON.stringify(result.gaps?.map((g) => g.type))}`);
    assert('C4-recovery-gap', result.gaps?.some((g) => g.type === 'nexus-crash-recovery'), `gaps=${JSON.stringify(result.gaps?.map((g) => g.type))}`);
}
...
async function c5() {
    ...
    const allTurnIdxs = (result.turns || []).filter((t) => typeof t.turnIdx === 'number').map((t) => t.turnIdx);
...
//# sourceMappingURL=test-p5-crash-recovery.js.map