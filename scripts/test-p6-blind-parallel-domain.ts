...
function mkFixture(id: string): string {
  ...
  if (fs.existsSync(KNOWN_ROLES_SRC)) {
    fs.writeFileSync(
      path.join(dir, '.claude', 'hooks', 'lib', 'known-roles.js'),
      fs.readFileSync(KNOWN_ROLES_SRC, 'utf8')
    );
  } else {
    fs.writeFileSync(
      path.join(dir, '.claude', 'hooks', 'lib', 'known-roles.js'),
      `module.exports = { KNOWN_ROLES: ['arki','jobs','riki','fin','ace','zero','edi','nova','sage','vera'] };`
    );
  }
...
function writeSession(dir: string, sess: object): void {
  fs.writeFileSync(
    path.join(dir, 'memory', 'sessions', 'current_session.json'),
    JSON.stringify(sess, null, 2) + '\n'
  );
}

function writeDispatchConfig(dir: string, config: object): void {
  fs.writeFileSync(
    path.join(dir, 'memory', 'shared', 'dispatch_config.json'),
    JSON.stringify(config, null, 2) + '\n'
  );
}

function buildInput(role: string, cwd: string): object {
  return {
    tool_name: 'Task',
    tool_input: {
...
function spawnHook(input: object): Promise<{ code: number; stdout: string; stderr: string }> {
...
function parseOutputPrompt(stdout: string): string | null {
  try {
...
async function d1() {
  ...
  assert('D1-no-domain-prepend',
    !prompt?.includes('blind-parallel 도메인 범위'),
    `domain prepend present=${prompt?.includes('blind-parallel 도메인 범위')}`);
}
