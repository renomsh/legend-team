...
const results = [];
let warnCount = 0;
function check(label, pass, warn, message) {
    const r = { label, pass, warn, message };
    results.push(r);
...
function findCommandInHookGroups(groups, targetFragment) {
    ...
    for (const group of groups) {
        ...
        for (const hook of group.hooks) {
            if (hook.command && hook.command.includes(targetFragment))
                return true;
        }
    }
    return false;
...
check('PreToolUse: pre-tool-use-task.js', preHookRegistered, !preHookRegistered, preHookRegistered
...
check('PostToolUse: post-tool-use-task.js', postHookRegistered, !postHookRegistered, postHookRegistered
...
check('SessionEnd hook', sessionEndRegistered, !sessionEndRegistered, sessionEndRegistered
...
// exit 0 — 검증만, 강제 종료 X
process.exit(0);
//# sourceMappingURL=validate-hook-registration.js.map