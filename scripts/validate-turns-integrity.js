...
function loadValidPhases() {
    const c = readJson(PHASE_CATALOG_PATH);
    return c?.phases?.map(p => p.id) ?? [];
}
function validateIntegrity(sessionId, turnsRaw, agentsCompletedRaw, legacy, validPhases) {
    const r = {
    ...
    turns.forEach((t, i) => {
        ...
        if (typeof t.turnIdx !== 'number') {
            r.errors.push(`${px}: turnIdx 누락/비숫자`);
            r.ok = false;
        }
        else {
            if (seenIdx.has(t.turnIdx)) {
                r.errors.push(`${px}: turnIdx=${t.turnIdx} 중복`);
                r.ok = false;
            }
        ...
        if (validPhases.length > 0 && t.phase !== undefined) {
            if (!validPhases.includes(t.phase)) {
                r.errors.push(`${px}: phase="${t.phase}" phase_catalog에 없음`);
                r.ok = false;
            }
        }
        if (t.recallReason !== undefined && !turn_types_1.VALID_RECALL_REASONS.includes(t.recallReason)) {
            r.errors.push(`${px}: 알 수 없는 recallReason="${t.recallReason}"`);
            r.ok = false;
        }
    });
    // RK-1: agentsCompleted ↔ turns[] 정합성
    if (Array.isArray(agentsCompletedRaw)) {
        const agents = agentsCompletedRaw;
        ...
        for (const role of allRoles) {
            ...
            if (tc !== ac) {
                ...
                if (tc > 1 && ac === 1) {
                    r.errors.push(`RK-1: role="${role}" turns=${tc}회 vs agentsCompleted=${ac}회 — ` +
                        `agentsCompleted이 Set 의미로 중복 제거됨 (D-048 "중복 허용 배열" 위반)`);
                }
                else {
                    r.errors.push(`RK-1: role="${role}" turns=${tc}회 vs agentsCompleted=${ac}회 — 정합성 불일치`);
                }
                r.ok = false;
            }
        }
        ...
        if (r.rolesFromTurns.length === agents.length) {
            for (let i = 0; i < agents.length; i++) {
                if (agents[i] !== r.rolesFromTurns[i]) {
                    r.warnings.push(`RK-1: 순서 불일치 [${i}] agentsCompleted="${agents[i]}" vs turns="${r.rolesFromTurns[i]}"`);
                }
            }
        }
    }
...
// RK-2: Smoke Test
// ──────────────────────────────────────────
function runSmokeTest(validPhases) {
    console.log('\n═══ Smoke Test (RK-2) ═══');
    // Fixture 1: phase 오타
...
// Main
// ──────────────────────────────────────────
function generateReport(results, outPath) {
    ...
    for (const r of results) {
        lines.push(`| ${r.sessionId} | ${r.turnsCount} | ${r.agentsCompletedCount} | ` +
...
function main() {
    ...
    const results = targets.map(s => validateIntegrity(String(s['sessionId'] ?? 'unknown'), s['turns'], s['agentsCompleted'], s['legacy'] === true, validPhases));
    ...
    if (reportFlagIdx >= 0 && args[reportFlagIdx + 1]) {
        generateReport(results, path_1.default.resolve(CWD, args[reportFlagIdx + 1]));
    }
    process.exit(failCount > 0 ? 1 : 0);
}
if (require.main === module)
    main();
//# sourceMappingURL=validate-turns-integrity.js.map