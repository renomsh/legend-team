...
check('scoringLag 전 필드 존재', reg.metrics.every(m => typeof m.scoringLag === 'number'));
check('windows=[20,100,500]', reg.metrics.every(m => m.windows.length === 3 && m.windows[0] === 20));
...
check('valid registry → errors=[]', (0, compute_growth_1.validateRegistry)(reg).length === 0);
const bad = {
    schemaVersion: '0.1.0', registryVersion: 'x', effectiveFrom: 'x',
    metrics: [
        { id: 'dup.a', tier: 'common', source: { type: 'x' }, scorer: { mode: 'automatic' }, scoringLag: -1, windows: [20], normalization: 'none', status: 'active' },
        { id: 'dup.a', tier: 'common', source: { type: 'x' }, scorer: { mode: 'automatic' }, scoringLag: 0, windows: [20], normalization: 'none', status: 'active' },
    ]
};
const errs = (0, compute_growth_1.validateRegistry)(bad);
...
const r052 = (0, compute_growth_1.resolveRegimeAt)('session_052', regimes);
check('session_052 (전 레짐) → null', r052.id === null && !r052.inTransitionZone);
const r053 = (0, compute_growth_1.resolveRegimeAt)('session_053', regimes);
check('session_053 → R-001 + zone', r053.id === 'R-001' && r053.inTransitionZone === true);
const r055 = (0, compute_growth_1.resolveRegimeAt)('session_055', regimes);
check('session_055 (transitionZone 내) → R-001 + zone', r055.id === 'R-001' && r055.inTransitionZone === true);
const r056 = (0, compute_growth_1.resolveRegimeAt)('session_056', regimes);
check('session_056 (zone 이탈) → R-001 + !zone', r056.id === 'R-001' && r056.inTransitionZone === false);
const r068 = (0, compute_growth_1.resolveRegimeAt)('session_068', regimes);
check('session_068 → R-003 + zone', r068.id === 'R-003' && r068.inTransitionZone === true);
const r078 = (0, compute_growth_1.resolveRegimeAt)('session_078', regimes);
...
let threw = false;
try {
    (0, compute_growth_1.scoreMetric)(aceMetric, 'session_078');
}
catch (e) {
    threw = e.message.includes('Phase 1');
}
check('scoreMetric throws Phase 1', threw);
threw = false;
try {
    (0, compute_growth_1.resolvePendingLag)((0, compute_growth_1.loadExistingOutput)(), 'session_078');
}
catch (e) {
    threw = e.message.includes('Phase 1');
}
...
(async () => {
    const out = await (0, compute_growth_1.computeGrowth)({ dryRun: true });
...
//# sourceMappingURL=verify-growth-phase0.js.map