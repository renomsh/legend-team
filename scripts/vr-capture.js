...
exports.FROZEN_TS_MS = exports.VIEWPORTS = exports.PAGES = void 0;
...
exports.FROZEN_TS_MS = new Date(process.env.VR_FROZEN_TS || '2026-01-01T00:00:00Z').valueOf();
async function applyMitigations(page) {
    ...
    await page.evaluate(() => {
        ...
        if (typeof window !== 'undefined' && window.echarts) {
            window.echarts.__animationDisabled = true;
        }
    });
...
async function captureBaseline(opts = {}) {
    ...
    const pages = opts.pages ?? exports.PAGES;
    const viewports = opts.viewports ?? exports.VIEWPORTS;
    const result = { pngs: [], bboxes: [], skipped: [] };
    if (opts.dryRun) {
    ...
    try {
      ...
      const _Date = Date;
      const _now = ${exports.FROZEN_TS_MS};
      ...
      Date = FrozenDate;
        ...
        for (const p of pages) {
            for (const vp of viewports) {
                ...
                const pngDir = (0, path_1.join)(baselineDir, p.slug);
                (0, fs_1.mkdirSync)(pngDir, { recursive: true });
                const pngPath = (0, path_1.join)(pngDir, `${vp.name}.png`);
                ...
                (0, fs_1.mkdirSync)(bboxDir, { recursive: true });
                const bboxPath = (0, path_1.join)(bboxDir, `${p.slug}-${vp.name}.json`);
                (0, fs_1.writeFileSync)(bboxPath, JSON.stringify(bboxes, null, 2));
...
//# sourceMappingURL=vr-capture.js.map